...
def test_constructionLinearNode():
    ...
    assert_true(node.node[LinearNode].name == 'name')
    assert_true(node.node[LinearNode].layer == 1)
    assert_true(node.node[LinearNode].pytorchLayerId == pytorchLayerId)
    assert_true(node.node[LinearNode].maxNumInFeatures == 32)
